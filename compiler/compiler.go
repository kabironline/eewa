...
func defineBuiltins(s *SymbolTable) {
	builtins := []string{"len", "first", "last", "rest", "push", "puts"}
...
func (c *Compiler) Compile(node ast.Node) error {
	...
	case *ast.IfExpression:
		...
		// Emit an `OpJumpNotTruthy` with a dummy value 9999
		...
		// Emit an `OpJump` with a dummy value 9999
	...
	case *ast.HashLiteral:
		...
		// Sort keys to ensure consistent order because go maps are unordered
